...
def Recommend(request):
    ...
    movies_list = Movies.objects.values('movie_id', 'movie_type_list')
    i = 0
    movies = defaultdict()
    ...
    max_index = list(pd.Series(similarity).sort_values(ascending=False).index[:12])
    ...
    for i in max_index:
        movie_id = movies_list[i]['movie_id']
        max_index_movie_id.append(movie_id)

    # response['movie_id'] = max_index_movie_id

    recommend_movies = Movies.objects.filter(movie_id__in=max_index_movie_id)
